...
import ru.practicum.event.storage.CustomSearchEventRepository;
...
import static ru.practicum.event.state.UserStateAction.SEND_TO_REVIEW;
...
public class EventServiceImpl implements EventService {
    ...
    @Transactional
    @Override
    public EventFullDto updateOwner(Long userId, Long eventId, UpdateEventUserRequest request) {
        ...
        if (CANCEL_REVIEW.equals(request.getStateAction())) {
            oldEvent = eventMapper.toEvent(oldEvent, request, category, location);
    ...
    @Transactional
    @Override
    public EventFullDto updateAdmin(UpdateEventAdminRequest request, Long eventId) {
        ...
        if (PUBLISH_EVENT.equals(request.getStateAction())) {
            if (event.getEventState().equals(PENDING)) {
                event = eventMapper.toEvent(event, request, category, location);
    ...
    @Transactional(readOnly = true)
    @Override
    public EventFullDto getByRequest(Long eventId, HttpServletRequest request) {
        Event event = getEvent(eventId);
    ...
    @Transactional(readOnly = true)
    @Override
    public List<EventShortDto> getByOwnerId(Long userId, Integer from, Integer size) {
    ...
    private void sendInfoAboutViewInStats(List<Long> eventsIds, HttpServletRequest request) {
        for (Long id : eventsIds) {
            statsClient.createHit(new EndpointHitDto("ewm-main-service", "/events/" + id,
